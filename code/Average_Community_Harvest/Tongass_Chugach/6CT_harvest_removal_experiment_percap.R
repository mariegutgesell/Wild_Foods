...
ggplot(total_harvest_rank, aes(x = harvest_rank, y= Percapita_Pounds_Harvested_sum_avg, fill = Habitat)) +
  geom_col() +
...
total_harvest_rank %>%
  ...
  ggplot(aes(x = harvest_rank, y= Percapita_Pounds_Harvested_sum_avg, fill = Habitat)) +
  geom_col() +
  scale_fill_manual(values = c("#FF9999","#003366","#CC9966", "#339933"))+
  theme_classic() +
  facet_wrap(~Site) +
...
harvest_div <- read.csv("data/intermediate_data/harvest_diversity_metrics.csv")
