...
df_comm_avg <- df %>%
  dplyr::select(Site_Year_Code, Habitat, Trophic_Level, Trophic_Category, Lowest_Common_Taxon_Name, Estimated_Total_Pounds_Harvested_sum, Percapita_Pounds_Harvested_sum) %>% ##total harvest proportion and percapita harvest proportion are the same
  ...
  group_by(Site, Habitat, Trophic_Level, Trophic_Category, Lowest_Common_Taxon_Name) %>%
