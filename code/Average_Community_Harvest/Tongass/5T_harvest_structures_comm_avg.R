...
##Harvest Structure for Metlaktla ------------------
met <- df_comm_avg %>%
...
tl <- tl %>%
  mutate(Resource_cat = c(14:19, 36:48, 24:31, 1:9))
...
##Harvest Structure for Yakutat -------------------
yakutat <- df_comm_avg %>%
  filter(Site == "Yakutat") %>%
  filter(!is.na(Trophic_Level)) ##eel don't know trophic level, so for now just removing -- NEED to come back to this
...
tl <- tl %>%
  mutate(Resource_cat = c(24:34,45:68,35:44, 1:23))

tl[nrow(tl) + 1,] = list("Human", "5", "NA", 35)
...
##Harvest Structure for Metlaktla w/ groups as trophic categories and habitat------------------
met <- df_comm_avg %>%
  filter(Site == "Metlakatla") %>%
  group_by(Habitat, Trophic_Category, Trophic_Level) %>%
  summarise_at(vars(Percapita_Pounds_Harvested_sum_avg), list(total_percap = sum)) %>%
  unite(Resource, c("Habitat", "Trophic_Category"), sep = "_", remove = FALSE)
...
tl <- tl %>%
  mutate(Resource_cat = c(13:14, 24:29, 16:19, 1:4))
...
#Set edge width based on weight
E(net)$width <- E(net)$total_percap
...
####Harvest Structure for Yakutat w/ groups as trophic categories and habitat------------------
yak <- df_comm_avg %>%
  filter(Site == "Yakutat") %>%
  group_by(Habitat, Trophic_Category, Trophic_Level) %>%
...
tl <- tl %>%
  mutate(Resource_cat = c(13:15, 28:33, 20:23, 1:5))
...
#Set edge width based on weight
E(net)$width <- E(net)$total_percap/3
...
####Harvest Structure for Klukwan w/ groups as trophic categories and habitat------------------
kluk <- df_comm_avg %>%
  filter(Site == "Klukwan") %>%
  group_by(Habitat, Trophic_Category, Trophic_Level) %>%
  filter(!is.na(Trophic_Level)) %>%
...
tl <- tl %>%
  mutate(Resource_cat = c(13,15,17, 28:32, 23:25, 1:4))
...
####Harvest Structure for Hoonah w/ groups as trophic categories and habitat------------------
hoonah <- df_comm_avg %>%
  filter(Site == "Hoonah") %>%
  group_by(Habitat, Trophic_Category, Trophic_Level) %>%
  filter(!is.na(Trophic_Level)) %>%
...
tl <- tl %>%
  mutate(Resource_cat = c(13,15,17, 29:34, 23:26, 1:4))
