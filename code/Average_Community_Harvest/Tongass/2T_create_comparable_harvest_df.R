##Determining lowest common taxonomic unit across communities/across years for Tongass
...
df_final <- read.csv("data/intermediate_data/tongass_harvest_data_clean.csv") ##csv from 1T_dataframe_formation.R
...
test <- df_final %>%
  filter(Taxa_lvl3 == "Berries")
str(df)
##the difference in number of rows in harvest df and df w/ lowest common taxon is the removal of coal and mushrooms, which are not comparable (only part of surveys after 2000)
...
unique_trophic_common_taxa <- df %>%
  ungroup() %>%
  select(Site_Year_Code,Taxa_lvl1, Taxa_lvl2, Lowest_Common_Taxon_Name, Habitat:Trophic_Category) %>%
  distinct()
...
mean_func <- function(x){
  ...
  x_final <- full_join(x1, x2, by = c("Site_Year_Code", "Lowest_Common_Taxon_Name")) %>%
    full_join(x3, by = c("Site_Year_Code", "Lowest_Common_Taxon_Name")) %>%
    full_join(x4, by = c("Site_Year_Code", "Lowest_Common_Taxon_Name")) %>%
    full_join(x5, by = c("Site_Year_Code", "Lowest_Common_Taxon_Name"))
}
...
df_common_taxa <- left_join(df_mean, df_sum, by = c("Site_Year_Code", "Lowest_Common_Taxon_Name")) %>%
  left_join(unique_trophic_common_taxa, by = c("Site_Year_Code", "Lowest_Common_Taxon_Name")) %>%
  left_join(unique_surveys, by = "Site_Year_Code") %>%
  select(Site_Year_Code, Lowest_Common_Taxon_Name, Habitat:Est_Comm_Population, Percent_Using_mean:Estimated_Amount_Harvested_sum)
...
write.csv(df_common_taxa, "data/intermediate_data/tongass_comparable_harvest_df.csv")
