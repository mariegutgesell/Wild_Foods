##Tongass -- community average harvest phenology -- percapita harvest
...
simulated_harvest <- lapply(1:nrow(df_hp), function(i) {
  amount <- df_hp$Percapita_Pounds_Harvested_sum_avg[i]
...
##Calculate proportion of year that harvest is below threshold (total percapita harvest/365)
...
ggplot(comm_dv_cv, aes(x = synchrony_gross, y = harvest_total_cv))+
  geom_point() +
  geom_smooth(method = "lm") +
  theme_classic()

sync_cv_lm <- lm(harvest_total_cv ~ synchrony_gross, comm_dv_cv)
summary(sync_cv_lm)
...
ggplot(comm_dv_cv, aes(x = log(sd), y = synchrony_gross)) +
  geom_point() +
  geom_smooth(method = "lm") +
  theme_classic()

sd_h_sync_lm <- lm(synchrony_gross ~ log(sd), comm_dv_cv)
...
ggplot(comm_dv_cv, aes(x = log(sd), y = synchrony_loreau)) +
  geom_point() +
  geom_smooth(method = "lm") +
  theme_classic()

sd_h_sync_lm <- lm(synchrony_loreau ~ log(sd), comm_dv_cv)
summary(sd_h_sync_lm)
