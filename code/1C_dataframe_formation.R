##Generating cleaned dataframe of CSIS harvest data for communities around Chugach NF for food web analysis
##Wild Foods Project
...
fish <- fish %>%
 ...
 #   startsWith(Resource_Code, "1208") ~ "Blenny", ## not in chugach df
    startsWith(Resource_Code, "1210") ~ "Gadiformes", ##cod
  ...
  #  startsWith(Resource_Code, "1220") ~ "Lamprey", ## not in chugach df
    ...
    startsWith(Resource_Code, "1242") ~ "Wolffish",
    ...
    startsWith(Resource_Code, "1255") ~ "Pike",
  ...
  mutate(Taxa_lvl4 = case_when(
    startsWith(Resource_Code, "11") ~ "Oncorhynchus", ##salmon
    startsWith(Resource_Code, "1202") ~ "Herring",
 ...
 #   startsWith(Resource_Code, "1208") ~ "Blenny", ## not in chugach df
    ...
    startsWith(Resource_Code, "1212") ~ "Eel",
    startsWith(Resource_Code, "1214") ~ "Flounder",
    startsWith(Resource_Code, "1216") ~ "Hexagrammidae", ##greenling
    startsWith(Resource_Code, "1218") ~ "Halibut",
   # startsWith(Resource_Code, "1220") ~ "Lamprey",  ##not in chugach df
    startsWith(Resource_Code, "1222") ~ "Perch", 
    startsWith(Resource_Code, "1226") ~ "Rockfish",
  ...
  mutate(Taxa_lvl5 = case_when(
    ...
    startsWith(Resource_Code, "116") ~ "Landlocked Salmon", 
    startsWith(Resource_Code, "117") ~ "Spawnouts",
    #startsWith(Resource_Code, "118") ~ "Salmon Roe", ##no salmon roe in the chugach species list
 ...
 #   startsWith(Resource_Code, "120408") ~ "Surf Smelt", ##not in chugach df
#    startsWith(Resource_Code, "12041") ~ "Silver Smelt", ##not in chugach df
 ...
 #   startsWith(Resource_Code, "120699") ~ "Unknown Bass", ##not in chugach df
 #   startsWith(Resource_Code, "1208") ~ "Blenny", ##not in chugach df
...
#    startsWith(Resource_Code, "121006") ~ "Pacific (silver) hake", ##not in chugach df
    startsWith(Resource_Code, "12109") ~ "Unknown Gadiformes",
    startsWith(Resource_Code, "1212") ~ "Eel",
    startsWith(Resource_Code, "121406") ~ "Starry Flounder",
 ...
 #   startsWith(Resource_Code, "12229") ~ "Unknown Perch",  ##not in chugach df
  ...
  #  startsWith(Resource_Code, "122614") ~ "Brown Rockfish",  ##not in chugach df
    startsWith(Resource_Code, "122630") ~ "Tiger Rockfish", 
    startsWith(Resource_Code, "122634") ~ "China Rockfish",
    startsWith(Resource_Code, "122616") ~ "Dusky Rockfish",
    startsWith(Resource_Code, "122646") ~ "Northern Rockfish",
    startsWith(Resource_Code, "122648") ~ "Boccaccio Rockfish",
    startsWith(Resource_Code, "122618") ~ "Copper Rockfish",
    startsWith(Resource_Code, "122678") ~ "Rougheye Rockfish",
    startsWith(Resource_Code, "12269") ~ "Unknown Rockfish",
    startsWith(Resource_Code, "1228") ~ "Sablefish (black cod)", 
  #  startsWith(Resource_Code, "123002") ~ "Buffalo Sculpin", ## not in chugach df
  #  startsWith(Resource_Code, "123004") ~ "Bullhead Sculpin",## not in chugach df
    #startsWith(Resource_Code, "12300600") ~ "Irish Lord",
    startsWith(Resource_Code, "1230069") ~ "Unknown Irish Lord",
    startsWith(Resource_Code, "12309") ~ "Unknown Sculpin",
  #  startsWith(Resource_Code, "123202") ~ "Dogfish", ## not in chugach df
   ...
   # startsWith(Resource_Code, "124002") ~ "Blue Fin", ## not in chugach df
    ...
    startsWith(Resource_Code, "125010") ~ "Lake Trout",
 #   startsWith(Resource_Code, "1250069") ~ "Dolly Varden - unknown", ##not in chugach df
    startsWith(Resource_Code, "1252") ~ "Grayling",
 #   startsWith(Resource_Code, "1254") ~ "Pike", ??
    startsWith(Resource_Code, "12549") ~ "Unknown Pike",
    startsWith(Resource_Code, "1255") ~ "Northern Pike",
   # startsWith(Resource_Code, "125802") ~ "Green Sturgeon", ##not in chugach df
  #  startsWith(Resource_Code, "125804") ~ "White Sturgeon",  ##not in chugach df
    ...
    startsWith(Resource_Code, "126408") ~ "Humpback Whitefish", 
    startsWith(Resource_Code, "12641") ~ "Lake Whitefish",
  ...
  mutate(Habitat = case_when(
    ...
    startsWith(Taxa_lvl3, "Pike") ~ "Freshwater_Anadromous",
    startsWith(Taxa_lvl3, "Burbot") ~ "Freshwater_Anadromous",
    startsWith(Taxa_lvl3, "Sturgeon") ~ "Freshwater_Anadromous",
    startsWith(Taxa_lvl3, "Sheefish") ~ "Freshwater_Anadromous",
    startsWith(Taxa_lvl3, "Lamprey") ~ "Freshwater_Anadromous",
  ...
  mutate(Roe_Collection_Type = case_when(
    ...
    startsWith(Resource_Code, "120308") ~ "Roe on Hair Seaweed", ##not in chugach df
    #startsWith(Resource_Code, "120310") ~ "Roe in Hemlock Branches", ##not in chugach df
  ))
...
##check the irish lord shit, and northern pike/pike -- what adds up? 
...
rm(fish, fish_str, fish1, fish2, fish3, fish4)
...
lm <- lm %>%
  ...
  mutate(Taxa_lvl2 = case_when(
    ...
    startsWith(Resource_Code, "23") ~ "Feral Animals",
  )) %>%
  mutate(Taxa_lvl3 = case_when( ##only small land mammals are broken down further 
    ...
    startsWith(Resource_Code, "2304") ~ "Cattle - Feral",
    startsWith(Resource_Code, "2306") ~ "Rabbit - Feral",
    startsWith(Resource_Code, "2308") ~ "Reindeer - Feral",
  )) %>%
  ...
  mutate(Taxa_lvl5 = case_when( ##only small mammals broken down further
    ...
    startsWith(Resource_Code, "2224") ~ "Muskrat",
    startsWith(Resource_Code, "2226") ~ "Porcupine", 
    startsWith(Resource_Code, "222802") ~ "Parka Squirrel (ground)",
    startsWith(Resource_Code, "222804") ~ "Tree Squirrel",
   # startsWith(Resource_Code, "222899") ~ "Unknown Squirrel", ##not in chugach df
  #  startsWith(Resource_Code, "222806") ~ "Flying Squirrel", #not in chugach df
    startsWith(Resource_Code, "2230") ~ "Weasel",
    startsWith(Resource_Code, "2232") ~ "Wolf",
    startsWith(Resource_Code, "2234" ) ~ "Wolverine",
    startsWith(Resource_Code, "2120") ~ "Muskox",
...
be <- be %>%
  ...
  mutate(Taxa_lvl3 = case_when(
   ...
   # startsWith(Resource_Code, "4306") ~ "Swan Eggs", ##not in chugach df
  #  startsWith(Resource_Code, "4308") ~ "Crane Eggs",##not in chugach df
    startsWith(Resource_Code, "4310") ~ "Shorebird Eggs",
    startsWith(Resource_Code, "4312") ~ "Seabirds and Loon Eggs",
 #   startsWith(Resource_Code, "4318") ~ "Upland Game Bird Eggs", ##not in chugach df 
  ...
  mutate(Taxa_lvl4 = case_when(
    ...
    startsWith(Resource_Code, "410206") ~ "Eider",
   ...
   # startsWith(Resource_Code, "410222") ~ "Redhead Duck", ##not in chugach df
    startsWith(Resource_Code, "410226") ~ "Scaup", 
    startsWith(Resource_Code, "410228") ~ "Scoter", 
    startsWith(Resource_Code, "410230") ~ "Northern Shoveler", 
  ...
  #  startsWith(Resource_Code, "410699") ~ "Unknown Swan", ##not in chugach df
    startsWith(Resource_Code, "410802") ~ "Sandhill Crane",
    startsWith(Resource_Code, "411002") ~ "Wilson's Snipe",
#    startsWith(Resource_Code, "411004") ~ "Black Oystercatcher",##not in chugach df
#    startsWith(Resource_Code, "411099") ~ "Unknown Shorebirds", ##not in chugach df
    startsWith(Resource_Code, "411204") ~ "Cormorants",
   # startsWith(Resource_Code, "411208") ~ "Grebe", ##not in chugach df
  #  startsWith(Resource_Code, "411210") ~ "Guillemot",##not in chugach df
    startsWith(Resource_Code, "411212") ~ "Gull",  
    startsWith(Resource_Code, "411214") ~ "Kittiwakes", 
    startsWith(Resource_Code, "411216") ~ "Loon",
    startsWith(Resource_Code, "411218") ~ "Murre", 
 ...
 #   startsWith(Resource_Code, "421899") ~ "Unknown Upland Game Birds", ##not in chugach df
 #   startsWith(Resource_Code, "430214") ~ "Mallard Eggs", #not in chugach df
    startsWith(Resource_Code, "430299") ~ "Unknown Duck Eggs",
  #  startsWith(Resource_Code, "430404") ~ "Canada Geese Eggs", #not in chugach df
    startsWith(Resource_Code, "430499") ~ "Unknown Geese Eggs",
 #   startsWith(Resource_Code, "43060") ~ "Swan Eggs", ##not in chugach df
 #   startsWith(Resource_Code, "43069") ~ "Unknown Swan Eggs", ##not in chugach df
#    startsWith(Resource_Code, "430802") ~ "Sandhill Crane Eggs",##not in chugach df
 #   startsWith(Resource_Code, "430899") ~ "Unknown Crane Eggs",##not in chugach df
    startsWith(Resource_Code, "431004") ~ "Black Oystercatcher Eggs",
   # startsWith(Resource_Code, "43109901" ) ~ "Unknown Small Shorebird Eggs", ##not in chugach df
  #  startsWith(Resource_Code, "43109902") ~ "Unknown Large Shorebird Eggs",##not in chugach df
  #  startsWith(Resource_Code, "431210") ~ "Guillemot Eggs",##not in chugach df
    startsWith(Resource_Code, "431212") ~ "Gull Eggs", ##
 #   startsWith(Resource_Code, "4312160") ~ "Loon Eggs", ##not in chugach df
#    startsWith(Resource_Code, "4312169") ~ "Unknown Loon Eggs",##not in chugach df
    startsWith(Resource_Code, "431218") ~ "Murre Eggs",  
    startsWith(Resource_Code, "431222") ~ "Puffin Eggs",
  ...
  #  startsWith(Resource_Code, "431802") ~ "Grouse Eggs", ##not in chugacah df
  #  startsWith(Resource_Code, "4318029") ~ "Unknown Grouse Eggs",##not in chugacah df
  #  startsWith(Resource_Code, "4318040") ~ "Ptarmigan Eggs",##not in chugacah df
  #  startsWith(Resource_Code, "4318049") ~ "Unknown Ptarmigan Eggs",##not in chugacah df
  )) %>%
  mutate(Taxa_lvl5 = case_when(
    startsWith(Resource_Code, "410202") ~ "Bufflehead",
    startsWith(Resource_Code, "410204") ~ "Canvasback",
    startsWith(Resource_Code, "41020602") ~ "Common Eider",
    startsWith(Resource_Code, "41020604") ~ "King Eider",
    startsWith(Resource_Code, "41020606") ~ "Spectacled Eider",
    startsWith(Resource_Code, "41020608") ~ "Stellar Eider",
    startsWith(Resource_Code, "4102069") ~ "Unknown Eider",
    startsWith(Resource_Code, "410208") ~ "Gadwall",
   # startsWith(Resource_Code, "41021002") ~ "Barrows Goldeneye", ##not in chugach df
  #  startsWith(Resource_Code, "41021004") ~ "Common Goldeneye", ##not in chugach df
    ...
    startsWith(Resource_Code, "410220") ~ "Northern Pintail",
  #  startsWith(Resource_Code, "410222") ~ "Redhead Duck", ##not in chugach df
  #  startsWith(Resource_Code, "41022602") ~ "Greater Scaup", ##not in chugach df
  #  startsWith(Resource_Code, "41022604") ~"Lesser Scaup",##not in chugach df
    startsWith(Resource_Code, "41022699") ~ "Unknown Scaup",
    startsWith(Resource_Code, "41022802") ~ "Black Scoter", 
  ...
  #  startsWith(Resource_Code, "4104041") ~ "Vancouver Canada Geese", ##not in chugach df
 ...
 #   startsWith(Resource_Code, "410699") ~ "Unknown Swan", ##not in chugach df
    startsWith(Resource_Code, "410802") ~ "Sandhill Crane",
    startsWith(Resource_Code, "41089") ~ "Unknown Crane",
    startsWith(Resource_Code, "411002") ~ "Wilson's Snipe",
  #  startsWith(Resource_Code, "411004") ~ "Black Oystercatcher", ##not in chugach df
  #  startsWith(Resource_Code, "41109901") ~ "Unknown Small Shorebirds", ##not in chugach df
  #  startsWith(Resource_Code, "41109902") ~ "Unknown Large Shorebirds",##not in chugach df
    startsWith(Resource_Code, "41120402") ~ "Double-Crested Cormorant",
    startsWith(Resource_Code, "41120404") ~ "Pelagic Cormorant",
    startsWith(Resource_Code, "4112049") ~ "Unknown Cormorant",
   # startsWith(Resource_Code, "41120802") ~ "Horned Grebe", ##not in chugach df
  #  startsWith(Resource_Code, "41120804") ~ "Red Necked Grebe",##not in chugach df
  #  startsWith(Resource_Code, "41120899") ~ "Unknown Grebe",##not in chugach df
   # startsWith(Resource_Code, "411210") ~ "Guillemot",##not in chugach df
    startsWith(Resource_Code, "4112129") ~ "Unknown Gull",  
    startsWith(Resource_Code, "41121402") ~ "Black Legged Kittiwake", 
    startsWith(Resource_Code, "4112160") ~ "Loon",
    startsWith(Resource_Code, "4112169") ~ "Unknown Loon",
    startsWith(Resource_Code, "41121802") ~ "Common Murre", 
    startsWith(Resource_Code, "4112189") ~ "Unknown Murre", 
    startsWith(Resource_Code, "41122202") ~ "Horned Puffin",
    startsWith(Resource_Code, "41122204") ~ "Tufted Puffin",
    startsWith(Resource_Code, "4112229") ~ "Unknown Puffin",
   # startsWith(Resource_Code, "411226") ~ "Tern", ## not in chugach df
    startsWith(Resource_Code, "411299") ~ "Unknown Seabird",
    startsWith(Resource_Code, "4114") ~ "Great Blue Heron",
    startsWith(Resource_Code, "42180202") ~ "Spruce Grouse", 
    startsWith(Resource_Code, "42180204") ~ "Sharp-Tailed Grouse",
    startsWith(Resource_Code, "42180206") ~ "Ruffed Grouse",
    startsWith(Resource_Code, "4218029") ~ "Unknown Grouse",
    startsWith(Resource_Code, "42180406") ~ "White-tailed Ptarmigan", 
 #   startsWith(Resource_Code, "42180404") ~ "Willow Ptarmigan", ##not in chugach df
    startsWith(Resource_Code, "4218049") ~ "Unknown Ptarmigan",
  #  startsWith(Resource_Code, "421899") ~ "Unknown Upland Game Birds", ##not in chugach df
    startsWith(Resource_Code, "430214") ~ "Mallard Eggs", ##not in chugach df
    startsWith(Resource_Code, "430299") ~ "Unknown Duck Eggs",
  #  startsWith(Resource_Code, "43040408") ~ "Lesser Canada Geese Eggs",##not in chugach df
  #  startsWith(Resource_Code, "4304049") ~ "Unknown Canada Geese Eggs",##not in chugach df
    startsWith(Resource_Code, "430499") ~ "Unknown Geese Eggs",
  #  startsWith(Resource_Code, "43060") ~ "Swan Eggs",##not in chugach df
   # startsWith(Resource_Code, "43069") ~ "Unknown Swan Eggs",##not in chugach df
    #startsWith(Resource_Code, "430802") ~ "Sandhill Crane Eggs",##not in chugach df
  ...
  #  startsWith(Resource_Code, "43109902") ~ "Unknown Large Shorebird Eggs",##not in chugach df
   # startsWith(Resource_Code, "431210") ~ "Guillemot Eggs",##not in chugach df
   # startsWith(Resource_Code, "43121204") ~ "Glaucous Winged Gull Eggs", ####not in chugach df
    startsWith(Resource_Code, "43121299") ~ "Unknown Gull Eggs",
 #   startsWith(Resource_Code, "4312160") ~ "Loon Eggs", ##not in chugach df
#    startsWith(Resource_Code, "4312169") ~ "Unknown Loon Eggs",##not in chugach df
    startsWith(Resource_Code, "4312189") ~ "Unknown Murre Eggs", 
    startsWith(Resource_Code, "4312229") ~ "Unknown Puffin Eggs",
  #  startsWith(Resource_Code, "4312260") ~ "Tern Eggs",
    startsWith(Resource_Code, "4312269") ~ "Unknown Tern Eggs",
...
mi <- mi %>%
  ...
  mutate(Taxa_lvl3 = case_when(
    ...
    startsWith(Resource_Code, "5036") ~ "Snails",
 #   startsWith(Resource_Code, "5037") ~ "Starfish", ##not in chugach df
    startsWith(Resource_Code, "5038") ~ "Squid",
    startsWith(Resource_Code, "5040") ~ "Whelk",
  #  startsWith(Resource_Code, "5099") ~ "Unknown Marine Invertebrates", ##not in chugach df
  )) %>%
  mutate(Taxa_lvl4 = case_when(
    startsWith(Resource_Code, "5002") ~ "Abalone",
    ...
    startsWith(Resource_Code, "501006") ~ "Hair Crab",
...
#    startsWith(Resource_Code, "5099") ~ "Unknown Marine Invertebrates", ##not in chugach df
  ...
  mutate(Taxa_lvl5 = case_when(
    ...
    startsWith(Resource_Code, "500610") ~ "Pinkneck Clam",
    startsWith(Resource_Code, "500612") ~ "Razor Clam",
    startsWith(Resource_Code, "500614") ~ "Softshell Clam",
    startsWith(Resource_Code, "50069") ~ "Unknown Clam",
 #   startsWith(Resource_Code, "500802") ~ "Basket Cockle", ##not in chugach df
#    startsWith(Resource_Code, "500804") ~ "Heart Cockle", ##not in chugach df
    startsWith(Resource_Code, "500899") ~ "Unknown Cockle",
    startsWith(Resource_Code, "501002") ~ "Box Crab",
    startsWith(Resource_Code, "501004") ~ "Dungeness Crab",
    startsWith(Resource_Code, "501006") ~ "Hair Crab",
  #  startsWith(Resource_Code, "50100802") ~ "Blue King Crab",  ##not in chugach df
  #  startsWith(Resource_Code, "50100804") ~ "Brown King Crab", ##not in chugach df
   # startsWith(Resource_Code, "50100808") ~ "Red King Crab", ##not in chugach df
    startsWith(Resource_Code, "50100899") ~ "Unknown King Crab",
    startsWith(Resource_Code, "50101202") ~ "Tanner Crab, Bairdi", 
    startsWith(Resource_Code, "5010129") ~ "Unknown Tanner Crab",
    startsWith(Resource_Code, "50109") ~ "Unknown Crab",
    startsWith(Resource_Code, "5012") ~ "Geoduck Clam",
    startsWith(Resource_Code, "5014") ~ "Limpet",
  #  startsWith(Resource_Code, "502002") ~ "Blue Mussel", ##not in chugach df
    startsWith(Resource_Code, "502099") ~ "Unknown Mussel",
    startsWith(Resource_Code, "5022") ~ "Octopus",
    startsWith(Resource_Code, "502402") ~ "Rock Oyster",
    startsWith(Resource_Code, "50249") ~ "Unknown Oyster",
    startsWith(Resource_Code, "502602") ~ "Weathervane Scallop",
  #  startsWith(Resource_Code, "502604") ~ "Rock Scallops", ##not in chugach df
    startsWith(Resource_Code, "502699") ~ "Unknown Scallop",
  #  startsWith(Resource_Code, "503004") ~ "Yein Sea Cucumber", ##not in chugach df
    startsWith(Resource_Code, "503099") ~ "Unknown Sea Cucumber",
  #  startsWith(Resource_Code, "503202") ~ "Green Sea Urchin", ##not in chugach df
  #  startsWith(Resource_Code, "503204") ~ "Red Sea Urchin", ##not in chugach df
  #  startsWith(Resource_Code, "504306") ~ "Purple Sea Urchin", ##not in chugach df
    startsWith(Resource_Code, "50329") ~ "Unknown Sea Urchin",
    startsWith(Resource_Code, "5034") ~ "Shrimp",
    startsWith(Resource_Code, "5036") ~ "Snails",
 #   startsWith(Resource_Code, "5037") ~ "Starfish", ##not in chugach df
 ...
 #   startsWith(Resource_Code, "5099") ~ "Unknown Marine Invertebrates", ##not in chugach df
  ...
  mutate(Habitat = case_when(
    ...
    grepl("Hair Crab", Taxa_lvl4) ~ "Marine",
...
veg <- veg %>%
  ...
  #  startsWith(Resource_Code, "601028") ~ "Thimbleberry",
    ...
    startsWith(Resource_Code, "601036") ~ "Bearberry",
...
sp_list <- df_final %>%
  ungroup() %>%
  dplyr::select(Taxa_lvl1:Taxa_lvl5, Habitat) %>%
  distinct()
write.csv(sp_list, "data/chugach_harvest_species_list.csv")
...
sp_list_2 <- df_final_2 %>%
  ...
  dplyr::select(Taxa_lvl1:Taxa_lvl5, Habitat) %>%
  distinct()
write.csv(sp_list_2, "data/chugach_harvest_species_list_harvested_taxa.csv")
