...
r_fig_2 <- ggplot(r_df, aes(x = species_removed, y = total_harvest, group = community)) +
  ...
  scale_color_gradient2(low = "deepskyblue",mid = "darkgoldenrod1", high = "brown3", midpoint = 0.3 ) +
  theme_classic() +
  ylab("Percapita Harvest Remaining\n(kg/person)") +
  xlab("Number of Species Removed") +
#  scale_y_log10()+
  theme(axis.text.x = element_text(size = 12),axis.text.y = element_text(size = 12),axis.title.y=element_text(size = 14), axis.title.x = element_text(size = 14), text = element_text(family = "Avenir"), strip.background = element_blank(), legend.position = "right")
...
df_temp_avg <- df_temp_avg %>%
  filter(Site_Year_Code %in% df_1$Site_Year_Code)
...
mid_cv <- harvest_cv %>%
  ungroup() %>%
  mutate(midpoint = min(cv_ph) + max(cv_ph) / 2)


#ia_cv <- left_join(df_1, harvest_cv, by = "Site")

ia_cv_fig <- ggplot(harvest_cv, aes(x = Year, y = total_harvest, group = Site)) +
  #  geom_point()+
  geom_line(aes(color = cv_ph), size = 1) +
...
##Join together
cv_r_fig <- ggarrange(r_fig_2, cv_fig, nrow = 1, ncol = 2, labels = c("a)", "b)"), font.label = list(colour = "black", size = 14, family = "Avenir"))
cv_r_fig
