...
ggplot() +
  geom_line(data = total_harvest_habitat, aes(x = date, y = harvest_total, group = habitat, color = habitat)) +
...
seasonal_aov <- aov(CV ~ cv_type, s_df)
summary(seasonal_aov)

d_df <- harvest_cv_all %>%
  filter(time_type == "Decadal")
decadal_aov <- aov(CV ~ cv_type, d_df)
summary(decadal_aov)
