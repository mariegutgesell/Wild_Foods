...
df <- read.csv("data/intermediate_data/comparable_harvest_df.csv") %>%
  filter(Site_Year_Code != "Hoonah_2016") ##removing b/c lauren said this survey was differently done, didn't ask about everything

##
test <- df %>%
  filter(Lowest_Common_Taxon_Name == "Muskox")
