...
source("code/Food_From_Forest_HarvestEstimates_SankeyFig.R")

##using d_f2 here 
df_spr <- df_2 %>%
  select(Site_Year_Code, Resource_Code, Resource_Name, Percapita_Pounds_Harvested, `2022_population`) %>%
  ...
  filter(est_total_lbs_2022 != "0") ##remove species with 0 harvest

##there is probably a better way to do this but for now lets just do it this way
##start with fish
###1) FISH ------------------
fish_code <- "1"
fish <- df_spr %>% 
...
fish_sp_list <- fish %>%
  ...
  group_by(Habitat, General_Category, General_Category_lvl2, Family, Species) %>%
  summarise_at(vars(est_total_kgs_2022), sum) %>%
...
veg_sp_list <- veg %>%
  ...
  filter(Family != "Wood") %>%
  filter(Family != "Kelp for Fertilizer") %>%
  filter(Family != "Coal")

##join all of these back together to get list of unique species


sp_list_all <- rbind(fish_sp_list, lm_sp_list, mm_sp_list, be_sp_list, mi_sp_list, veg_sp_list)
##157 species

habitat_sp <- sp_list_all %>%
  group_by(Habitat) %>%
  count()

##total directly supported species:
82 + 13 + 25
120/157
