##Tongass -- community average harvest phenology -- proportion of harvest
...
simulated_harvest_prop <- lapply(1:nrow(df_hp), function(i) {
  amount <- df_hp$Total_Harvest_prop[i]
...
simulated_harvest_prop_df <- do.call(rbind, simulated_harvest_prop) %>%
  mutate(month = lubridate::month(date, label = TRUE))

ggplot(simulated_harvest_prop_df, aes(x = date, y = harvest_amount, color = habitat)) +
  geom_line() +
...
ggplot(simulated_harvest_prop_df, aes(x = date, y = harvest_amount, group = habitat, color = habitat)) +
  geom_line() +
  scale_colour_manual(values = c("#FF9999","#003366","#CC9966", "#339933"))+
...
total_simulated_harvest_prop <- simulated_harvest_prop_df %>%
  filter(!is.na(harvest_amount)) %>%
  group_by(site, date) %>%
  summarise_at(vars(harvest_amount), list(harvest_total = sum))


ggplot(total_simulated_harvest_prop, aes(x = date, y = harvest_total, group = site, color = site)) +
  geom_line() +
  labs(x = "Date", y = "Harvest Amount (%)", title = "Total Harvest Amounts Across Harvest Season") +
  theme_classic()
  
total_harvest_all <- total_simulated_harvest_prop %>%
  filter(!is.na(harvest_total)) %>%
  group_by(date) %>%
  summarise_at(vars(harvest_total), list(harvest_mean = mean))

ggplot() +
  geom_line(data = total_harvest_all, aes(x = date, y = harvest_mean), color = "red", size = 3) +
  geom_line(data = total_simulated_harvest_prop, aes(x = date, y = harvest_total, group = site), color = "black") +
  labs(x = "Date", y = "Harvest Amount (%)", title = "Simulated Harvest Amounts Across Harvest Season") +
  theme_classic()
...
ggplot() +
  geom_line(data = simulated_harvest_prop_df, aes(x = date, y = harvest_amount, group = habitat, color = habitat)) +
  geom_line(data = total_simulated_harvest_prop, aes(x = date, y = harvest_total)) +
  scale_colour_manual(values = c("#FF9999","#003366","#CC9966", "#339933"))+
  labs(x = "Date", y = "Harvest Amount (%)", title = "Simulated Harvest Amounts Across Harvest Season") +
...
total_simulated_harvest_prop <- total_simulated_harvest_prop %>%
  group_by(site) %>%
  mutate(harvest_day = rank(harvest_total, ties.method = "min"))


ggplot(total_simulated_harvest_prop, aes(x = harvest_day, y = harvest_total, group = site)) +
  geom_line() +
...
harvest_threshold <- total_simulated_harvest_prop %>%
  group_by(site) %>%
...
harvest_threshold_2 <- total_simulated_harvest_prop %>%
  group_by(site) %>%
  mutate(below_harvest_threshold = ifelse(harvest_total < 0.3, "Y", "N")) %>%
  filter(below_harvest_threshold == "Y") %>%
  count() %>%
  mutate(prop_days_below_threshold = (n/365)*100) 
...
##could also try the mean 25 percentile across all communities.. 

 
##Calculate variability metrics
##calculating sd and CV
sim_harv_cv <- total_simulated_harvest_prop %>%
  group_by(site) %>%
